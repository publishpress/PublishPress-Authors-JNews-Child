<?php
	...
	if ( function_exists( 'get_coauthors' ) ) {
	...
	} elseif ( function_exists( 'get_multiple_authors' ) ) {
		...
		if ( $coauthors && is_array( $coauthors ) ) {
			...
			foreach ( $coauthors as $coauthor ) {
				$authors[] = [
					'avatar' => $coauthor->get_avatar(80),
					'id'   => $coauthor->ID,
					'name' => $coauthor->display_name,
					'url'  => $coauthor->link,
					'desc' => $coauthor->description,
					'type' => $coauthor->is_guest() ? 'guest-author' : false,
				];
			}
		}
	} else {
...
<?php foreach ( $authors as $author ) : ?>
	<div class="jeg_authorbox">
		<div class="jeg_author_image">
			<?php echo isset($author['avatar']) ? $author['avatar'] : get_avatar( $author['id'], 80, null, $author['name'] ); ?>
		</div>
